...
public class BoardDao {
	...
	public Integer insert(Board board) {
		...
		String sql = "" + "insert into boards "
				+ "(board_title, board_content, board_writer, board_date, board_original_file_name, board_filesystem_name, board_content_type ) "
				+ "values(?, ?, ?, now(), ?, ?, ?)";
	...
	public List<Board> selectByPage(int pageNo, int rowsPerPage)  {
		String sql = ""
				+ "select board_no, board_title, board_writer, board_date, board_hitcount, board_original_file_name, board_filesystem_name, board_content_type "
				+ "from boards " + "order by board_no desc " + "limit ?, ?";
		
		List<Board> list = jdbcTemplate.query(
			sql, 
			new Object[] {(pageNo-1)*rowsPerPage, rowsPerPage},
			new RowMapper<Board>() { //익명객체
				@Override
	...
	public int update(Board board)  {
		...
		int rows = jdbcTemplate.update(
				sql, board.getTitle(), board.getContent(), board.getNo()
			);
		return rows;
	}
